...
<template>
  ...
  <header class=" shadow">
    <div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
      <h1 class="text-3xl font-bold text-gray-900 dark:text-gray-300">Upload Notes</h1>
    </div>
  </header>

  <!-- Main Content -->
  <main class="max-w-4xl mx-auto mt-10 dark:bg-gray-800 dark:text-gray-100 shadow-lg rounded-lg p-8">
    <form id="blogForm" class="space-y-6">
      ...
      <div >
        ...
        <select id="material-type" class="block w-full p-3 border border-gray-300 rounded-lg shadow-sm focus:ring-primary-500 focus:border-primary-500">
      ...
      <div>
        ...
        <select id="semester" v-model="selectedSemester" @change="selectedSubject=0" class="block w-full p-3 border border-gray-300 rounded-lg shadow-sm focus:ring-primary-500 focus:border-primary-500">
          <option value="0" disabled>Select Semester</option>
          <option v-for="semester in 8" :key="semester" :value="semester">Semester {{ semester }}</option>
        </select>
      </div>
      <div>
        <label for="subject">Subject</label>
        <select id="subject" v-model="selectedSubject" @change="console.log(selectedSubject)" class="block w-full p-3 border border-gray-300 rounded-lg shadow-sm focus:ring-primary-500 focus:border-primary-500">
          <option value="0" disabled>Select Subject</option>
          <option v-if="selectedSemester!=0" v-for="subjects in semStore.getSemSubjects(selectedSemester)" :key="subjects" :value="subjects[1]" >{{subjects[0]}}</option>
...
<!--        </div>-->
      ...
      <!-- Submit Button -->
      <div>
        <button type="button" onclick="displayBlogPost()"
    ...
    <div id="blogPostPreview" class="hidden mt-10 border-t pt-10">
      <h2 class="text-2xl font-bold text-gray-800 mb-4">📸 Blog Preview</h2>
      <div class="space-y-4">
        ...
        <p class="text-sm text-gray-500">
          Uploaded by: <span id="previewUploader"></span> on <span id="previewDate"></span>
        </p>
